            ...
            <div class="w-full max-w-full px-3 mt-0 mb-6">
                <div class="border-black/12.5 shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
                  <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0 mb-4">
                    <h6><center>Edit Dokter</center></h6>
                  </div>
                  <form action="{{ route('admin-updateDokter',  $dokter->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                      <label for="nama" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Nama Dokter:</label>
                      <input type="text" name="nama" class="w-full p-2 border rounded-md" style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;" value="{{$dokter->nama}}">
                    </div>
                    <div class="mb-4">
                        <label for="alamat" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Alamat:</label>
                        <input type="text" name="alamat" class="w-full p-2 border rounded-md" style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;" value="{{$dokter->alamat}}">
                    </div>
                    <div class="mb-4">
                      <label for="no_hp" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">No.HP:</label>
                      <input type="text" name="no_hp" class="w-full p-2 border rounded-md" style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;" value="{{$dokter->no_hp}}">
                    </div>
                    <select name="id_poli" class="p-2 border rounded-md" style="padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;">
                      @foreach($polis as $poli)
                          <option value="{{ $poli->id }}" @if($poli->id == $dokter->id_poli) selected @endif>Bertugas di poliklinik : {{ $poli->nama_poli }}</option>
                      @endforeach
                    </select>                               
                    <div class="mb-4 flex justify-center">
                        <button type="submit" class="btn btn-success shadow-soft-2xl rounded-lg bg-dark stroke-0 text-center xl:p-2.5" style="background-image: linear-gradient(to bottom right, #0c04ef, #8624c2); color:white;">Tambah dokter</button>
                    </div>
                </form>
