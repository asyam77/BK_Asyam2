            ...
            <div class="w-full max-w-full px-3 mt-0 mb-6">
                <div class="border-black/12.5 shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
                  <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0 mb-4">
                    <h6><center>Edit Poliklinik</center></h6>
                  </div>
                  <form action="{{ route('admin-updatePoli',  $poli->id) }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        <label for="nama_poli" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Nama poli:</label>
                        <input type="text" name="nama_poli" class="w-full p-2 border rounded-md" style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;" value="{{ $poli->nama_poli }}" required>
                    </div>
                    <div class="mb-4">
                        <label for="keterangan" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Keterangan:</label>
                        <input type="text" name="keterangan" class="w-full p-2 border rounded-md" style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;" value="{{ $poli->keterangan }}" required>
                    </div>
