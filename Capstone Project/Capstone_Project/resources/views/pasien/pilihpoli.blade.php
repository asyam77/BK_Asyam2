            ...
            <div class="w-full max-w-full px-3 mt-0 mb-6">
                <div class="border-black/12.5 shadow-soft-xl relative flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
                  <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0 mb-4">
                    <h6><center>Pilih Poliklinik</center></h6>
                  </div>

                  <form action="{{ route('pasien-politerpilih') }}" method="POST" class="bg-white p-8 rounded-lg shadow-md">
                    @csrf
                    <div class="mb-4">
                        <label for="poliName" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Pilih Poliklinik:</label>
                        <select name="poliName" class="p-2 border rounded-md" style="padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;">
                            @foreach($uniquePoliNames as $poliName)
                                <option value="{{ $poliName }}">{{ $poliName->poli->nama_poli }}</option>
                            @endforeach
                        </select>
                    </div>
