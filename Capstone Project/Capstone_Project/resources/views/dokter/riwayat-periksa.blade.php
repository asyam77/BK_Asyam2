        ...
        <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
          <nav>
            ...
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
              ...
              <li class="text-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']" aria-current="page">Riwayat Periksa</li>
            </ol>
                        ...
                        @foreach($periksa as $key => $p)
                          <tr>
                            ...
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                              <div class="flex px-2 py-1">
                                <div class="flex flex-col justify-center">
                                  <center><h6 class="mb-0 text-sm leading-normal">{{ $p->daftarPoli->pasien->nama }}</h6></center>
                                </div>
                              </div>
                            </td>
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{ $p->tgl_periksa }}</td>
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{ $p->catatan }}</td>
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{ $p->biaya_periksa }}</td>
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                              <div class="flex justify-center items-center">
                                <a href="{{ route('dokter-detail-riwayat-periksa', $p->id) }}" class="btn btn-warning mr-2 py-1 px-4" style="color:#b2b81c">Detail</a>
                                <form action="{{ route('dokter-destroy-riwayat-periksa', $p->id) }}" method="POST" style="display: inline;">
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger py-1 px-4" style="color:#e72c2c" onclick="return confirm('Apakah Anda yakin ingin menghapus jadwal periksa ini?')">Hapus</button>
                                </form>
                              </div>
                            </td>                          
                          </tr>
