                ...
                <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0">
                  <div class="flex flex-wrap mt-0 -mx-3">
                    ...
                    <div class="flex-none w-5/12 max-w-full px-3 mt-0 text-right lg:w-1/2 lg:flex-none">
                      <a href="{{ route('admin-createObat') }}" class="btn btn-success shadow-soft-2xl rounded-lg bg-dark stroke-0 text-center xl:p-2.5" style="background-image: linear-gradient(to bottom right, #0c04ef, #8624c2); color:white;">Tambah Obat</a>
                    </div>
                  </div>
                </div>
                @if(session('success'))
                          ...
                          <tr>
                            ...
                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                              <div class="flex justify-center items-center">
                                <a href="{{ route('admin-editObat', $obat->id) }}" class="btn btn-warning mr-2 py-1 px-4" style="color:#b2b81c">Edit</a>
                                <form action="{{ route('admin-destroyObat', $obat->id) }}" method="POST" style="display: inline;">
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-danger py-1 px-4" style="color:#e72c2c" onclick="return confirm('Apakah Anda yakin ingin menghapus obat ini?')">Hapus</button>
                                </form>
                              </div>
                            </td>                          
                          </tr>
